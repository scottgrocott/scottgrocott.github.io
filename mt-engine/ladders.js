...
export function tickLadders(dt) {
  // Guard 1: playerRig null (config reload tears down player mid-climb)
  if (!playerRig) {
    if (_climbing) _stopClimbing();
    return;
  }
  ...
  if (!_climbing) {
    ...
    // Guard 2: currentLadder was cleared (e.g. clearLadders called during reload)
    if (!_currentLadder) {
      _stopClimbing();
      return;
    }
    ...
    // Keep player snapped to ladder X/Z
    playerRig.position.x += ((_currentLadder.position.x + 0.4) - playerRig.position.x) * 0.3;
