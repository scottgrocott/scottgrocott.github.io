...
async function boot() {
  try {
    await initScene();
    buildLayers();
    buildPanel();      // builds channel layer list + hand section (preview always visible)
    initStylePanel();
    initPopup();
    ...
    renderer.setAnimationLoop(() => {
      ...
      tickAudio(dt);
      tickHand(dt);           // updates preview canvas + brush sound every frame
